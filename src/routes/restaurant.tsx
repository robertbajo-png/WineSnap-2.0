...
export const Route = createFileRoute("/restaurant")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Snap a wine list or paste it in — get sommelier picks tuned to your taste.",
      },
    ],
  }),
...
function RestaurantPage() {
  ...
  const confidenceLabel = (c?: string) =>
    c === "safe"
      ? t("restaurant.safe")
      : c === "stretch"
        ? t("restaurant.stretch")
        : t("restaurant.balanced");
                ...
                <div key={i} className="rounded-2xl border border-white/10 bg-card/50 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      {p.producer && (
                        <p className="mt-1 text-xs text-muted-foreground">{p.producer}</p>
                      )}
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <span className={`font-display text-lg ${scoreColor(p.match_score)}`}>
                  ...
                  <p className="mt-3 border-t border-white/5 pt-3 text-sm leading-relaxed text-foreground/80">
                    {p.reason}
                  </p>
