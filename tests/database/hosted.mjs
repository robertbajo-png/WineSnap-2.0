...
try {
  ...
  const wine = ok(
    await owner.client
      ...
      .insert({
        user_id: owner.id,
        wine_name: "Synthetic security test",
        is_public: false,
        share_id: legacy,
  ...
  await check("owner can read private wine", async () =>
    assert.equal(ok(await owner.client.from("wines").select("id").eq("id", wine.id)).length, 1),
  );
  await check("stranger cannot read private wine", async () =>
  ...
  await check("stranger cannot read owner private profile", async () =>
    assert.equal(
      ok(await stranger.client.from("profiles").select("*").eq("id", owner.id)).length,
      0,
    ),
  );
  await check("owner downloads private image", async () =>
    assert.ok(ok(await owner.client.storage.from("wine-labels").download(path)).size > 0),
  );
  await check("stranger cannot download private image", async () =>
    assert.ok((await stranger.client.storage.from("wine-labels").download(path)).error),
  );
  await check("anonymous cannot sign private image", async () =>
    assert.ok((await anon.storage.from("wine-labels").createSignedUrl(path, 60)).error),
  );
  ok(await owner.client.from("wines").update({ is_public: true }).eq("id", wine.id));
  await check("legacy share ID resolves through public API", async () =>
    assert.equal(
      ok(await anon.from("public_wines").select("id").eq("share_id", legacy)).at(0)?.id,
      wine.id,
    ),
  );
  ...
  await check("anonymous can download explicitly shared image", async () => {
    assert.ok(
      ok(await anon.storage.from("wine-labels").download(path, {}, { cache: "no-store" })).size > 0,
    );
  });
  await check("anonymous cannot sign even a shared image", async () =>
    assert.ok((await anon.storage.from("wine-labels").createSignedUrl(path, 60)).error),
  );
  await check("owner cannot issue bearer download links", async () =>
    assert.ok((await owner.client.storage.from("wine-labels").createSignedUrl(path, 60)).error),
  );
  await check("batch signing is denied", async () => {
  ...
  await check("unsharing blocks new signed URLs", async () =>
    assert.ok((await anon.storage.from("wine-labels").createSignedUrl(path, 60)).error),
  );
  await check("unsharing removes public share lookup", async () =>
    assert.equal(ok(await anon.from("public_wines").select("id").eq("share_id", legacy)).length, 0),
  );
  await check("same image path denies anonymous download immediately after unshare", async () =>
    assert.ok(
      (await anon.storage.from("wine-labels").download(path, {}, { cache: "no-store" })).error,
    ),
  );
  await check("owner still downloads image after unshare", async () =>
    assert.ok(
  ...
  await check("authenticated user sees public-profile relation only", async () => {
    ...
    assert.equal(rows[0].follower_id, owner.id);
  });
  await check("participant sees their own private relation", async () =>
    assert.equal(
  ...
  await check("atomic rate limiter allows exactly 20 of 32 requests", async () => {
    const results = await Promise.all(
      Array.from({ length: 32 }, () =>
        owner.client.rpc("consume_api_rate_limit", { _bucket: "analyze-wine" }),
      ),
    );
